...
def load_data(in_dir):
    ...
    train_data,train_label,valid_data,valid_label,Valid_label,pernums_valid,test_data,test_label,Test_label,pernums_test = cPickle.load(f)
...
def train():
    ...
    if not use_CBL:
        ...
        _, samples_per_cls = np.unique(np.argmax(train_label, axis=-1),
                                       return_counts=True)
        cross_entropy = CB_loss_tf(
            labels=tf.cast(Y, "float64"), logits=Ylogits,
            samples_per_cls=samples_per_cls, beta=beta, is_training=False)
        cost = CB_loss_tf(labels=tf.cast(Y, "float64"), logits=Ylogits,
